...
struct ObjDoc {
    ...
    /// Verbatim contiguous leading comment block (spec §"Comments":
    /// "Comments can appear anywhere in an .obj file. They are used to
    /// annotate the file; they are not processed."). Real-world exporters
    /// (Blender's `# Blender v2.93 OBJ File:`, Maya, Meshroom, the spec's
    /// own §"Comments" example `# 4 vertices` / `# 4 texture vertices` /
    /// `# 4 normals`) open the file with a header comment block; dropping
    /// it and substituting a synthetic `# OBJ generated by …` line is a
    /// visible round-trip diff even though the bytes after it are
    /// equivalent. We capture the run of comment / blank lines that
    /// precedes the first non-comment directive verbatim (each entry is
    /// one source line with its leading `#` retained, trailing CR
    /// stripped; blank separator lines are preserved as empty strings so
    /// the block's internal shape survives). The encoder re-emits this
    /// block in place of its synthetic header when the key is present,
    /// making the preamble byte-faithful for the common header-comment
    /// case. Comments embedded *after* the first directive aren't
    /// positionally anchored (the parser is line-oriented and discards
    /// them during `preprocess_lines`); only the leading block round-trips
    /// — a header is where virtually all real-world annotation lives.
...
/// Collect the file's leading comment block — the contiguous run of
/// comment (`#…`) and blank lines that precedes the first line carrying a
/// non-comment directive. Returns each line verbatim (leading `#`
/// retained for comment lines, empty string for a blank separator line,
/// trailing CR stripped). Trailing blank lines at the *end* of the block
/// (the gap between the header and the first directive) are dropped so a
/// re-emit doesn't accumulate growth across repeated round-trips — the
/// encoder reinserts a single separating newline of its own.
///
/// Only the leading block is captured: a line-oriented parser can't
/// faithfully re-anchor a comment that sits between two arbitrary
/// directives, and virtually all real-world annotation (exporter banner,
/// vertex/face counts) lives in the header. A file that opens directly
/// with a directive yields an empty vector and keeps the synthetic
/// encoder header.
fn extract_header_comments(text: &str) -> Vec<String> {
    let mut block: Vec<String> = Vec::new();
    for raw_line in text.split('\n') {
        let line = raw_line.strip_suffix('\r').unwrap_or(raw_line);
        let trimmed = line.trim_start();
        if trimmed.is_empty() {
            // Blank separator line inside (or after) the header block.
    ...
    // Drop trailing blank lines so repeated round-trips don't grow the
    // block; the encoder owns the single newline before the body.
...
fn parse_obj_doc(text: &str) -> Result<ObjDoc> {
    ...
    // Capture the leading comment block before `preprocess_lines` strips
    // every `#…` to end-of-line. The encoder replays it verbatim.
    doc.header_comments = extract_header_comments(text);
...
fn build_scene(doc: ObjDoc, generate_normals: NormalGeneration) -> Result<Scene3D> {
    ...
    // Spec §"Comments" — leading header comment block, surfaced verbatim
    // so the encoder can re-emit it in place of its synthetic banner.
    // Absent key (file opened with a directive) keeps the synthetic
    // header.
    if !doc.header_comments.is_empty() {
        scene.extras.insert(
            "obj:header_comments".to_string(),
...
pub fn serialize_obj_with_options(
    ...
    // Spec §"Comments" — replay the source's leading comment block
    // verbatim when the decoder captured one (Blender / Maya / Meshroom
    // banners, vertex/face counts), else fall back to a synthetic banner.
    // Empty / non-string entries are skipped lenient-loader style; a
    // present-but-all-empty array still falls through to the synthetic
    // header.
    let mut emitted_header = false;
    if let Some(serde_json::Value::Array(lines)) = scene.extras.get("obj:header_comments") {
        for entry in lines {
